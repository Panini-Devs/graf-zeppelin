...
use serenity::{
    all::{
        ChannelType, ExplicitContentFilter, MfaLevel, PremiumTier, PremiumType, VerificationLevel,
    },
...
async fn guild(context: &Context, message: &Message) -> CommandResult {
    ...
    let guild_owner = cached_guild
        .member(&context, cached_guild.owner_id)
        .await
        .unwrap()
        .user
        .tag();
    ...
    let guild_channels: Vec<_> = cached_guild.channels.values().collect();
    let guild_channels_all = guild_channels.len();
    let guild_channels_text = guild_channels
        .iter()
        .filter(|c| c.kind == ChannelType::Text)
        .count();
    let guild_channels_voice = guild_channels
        .iter()
        .filter(|c| c.kind == ChannelType::Voice)
        .count();
    let guild_emojis = cached_guild.emojis.len();
    let guild_emojis_animated = cached_guild
        .emojis
        .iter()
        .filter(|(_, e)| e.animated)
        .count();
    let guild_emojis_normal = cached_guild
        .emojis
        .iter()
        .filter(|(_, e)| !e.animated)
        .count();
    ...
    let guild_roles_sorted = cached_guild
        .roles
        .iter()
        .sorted_by_key(|&(_, r)| r.position)
        .rev();
    let guild_roles_map = guild_roles_sorted
        .filter(|&(_, r)| r.id.get() != guild_id_u64)
        .map(|(_, r)| &r.name)
        .join(" / ");
    let guild_role_count = cached_guild
        .roles
        .iter()
        .filter(|&(_, r)| r.id.get() != guild_id_u64)
        .count();
...
async fn user_avatars(context: &Context, message: &Message) -> CommandResult {
    ...
    if !guild_avatar.is_empty() {
        ...
        message
            .channel_id
            .send_message(&context, CreateMessage::new().embeds(vec![embed, embed2]))
            .await?;
    } else {
        let embed = CreateEmbed::new()
            .author(CreateEmbedAuthor::new(user_name.clone()).icon_url(user.face()))
